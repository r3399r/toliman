...
const C3S3VectorInLineAndDeterminant = () => (
  <div>
    <Page pageNum={34}>
      <div className={style.head}>3 直線的向量性質與二階行列式</div>
      <Concept num={1} title="平面上直線的參數式與標準式">
        <div className={style.conceptHead}>方向向量與法向量</div>
        <div>{'坐標平面上一條直線 $L$，'}</div>
        <div>{'(1) 與 $L$ 平行的向量稱為 $L$ 的方向向量，習慣記為 $\\vector v$'}</div>
        <div>{'(2) 與 $L$ 垂直的向量稱為 $L$ 的法向量，習慣記為 $\\vector n$'}</div>
        <div>{'註：方向向量與法向量均不唯一，可長、可短、可反方向'}</div>
        <br />
        ...
        <div>{'(1) $t$ 稱為參數，可以當作搭捷運搭了 $t$ 站，不同的 $t$ 就是不同的位置'}</div>
        <div>{'(2) 把參數消掉即可得到直線的標準式'}</div>
        <br />
        <br />
        <br />
        <br />
    ...
    <Page pageNum={38}>
      <Concept num={2} title="二階行列式的定義與性質">
        ...
        <div>{'規定 $\\begin{vmatrix}a&b\\\\c&d\\end{vmatrix}=ad-bc$'}</div>
        <br />
        <div className={style.conceptHead}>二階行列式的性質</div>
        <div>{'(1) 行列互換，其值不變。如：'}</div>
        <br />
        <br />
        <div>{'(2) 兩行(列)對調，其值變號。如：'}</div>
        <br />
        <br />
        <div>{'(3) 任一行(列)的公倍數可以提出。如：'}</div>
        <br />
        <br />
        <div>{'(4) 將一行(列)乘一常數家到另一行(列)，其值不變。如：'}</div>
        <br />
        <br />
        <div>{'(5) 若一行(列)的各項均為兩項之和，則可拆開乘兩個行列式相加'}</div>
        <br />
        <br />
      </Concept>
      <Example num={7} ids={['17cb58623b1', '17cb5880bef']} height="5.3cm" />
    </Page>
    <Page pageNum={39}>
    ...
    <Page pageNum={40}>
      <Concept num={3} title="二階行列式的應用">
        <div className={style.conceptHead}>兩向量所張成平行四邊形面積</div>
        <div>
          {
            '兩向量 $\\vector a=(x_1,y_1)$ 與 $\\vector b=(x_2,y_2)$ 夾角 $\\theta$，設所張成的平行四邊形面積為 $A$， $A=|\\vector a||\\vector b|\\sin\\theta$，可推得 $A=\\sqrt{|\\vector a|^2|\\vector b|^2-(\\vector a\\cdot\\vector b)^2}=|\\begin{vmatrix}x_1&y_1\\\\x_2&y_2\\end{vmatrix}|$。將平行四邊形除以 $2$，即為所張成的三角形面積。'
          }
        </div>
        <br />
        <br />
