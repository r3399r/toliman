...
const C1S3LimitOfFunction = () => (
  <MathJax>
    <Page pageNum={12}>
      <div className={style.head}>1-3 函數及其極限</div>
      <Concept num={1} title="函數的概念">
        <div>
          {
            '函數值 $y$ 隨著變數 $x$ 而變化，對每個有意義的 $x$ 值皆可找到唯一的 $y$ 值作為對應，則稱 $y$ 是 $x$ 的函數，習慣記為 $y=f(x)$。$x$ 的取值範圍為「定義域」，$y$ 值的範圍為「值域」。'
          }
        </div>
        <br />
        ...
        <div>
          {'小於或等於 $x$ 的最大整數，記為 $[x]$，即為高斯函數，其圖形類似朝右上遞增的階梯。'}
        </div>
        <br />
    ...
    <Page pageNum={16}>
      ...
      <Concept num={3} title="函數極限的性質">
        <div>{'已知函數 $f(x)$ 與 $g(x)$ 在 $x$ 靠近 $a$ 的極限值皆存在，$c$ 為常數，則：'}</div>
        <div>
          {
        ...
        <div>
          {'2. $\\displaystyle\\lim_{n\\to a}[c\\cdot f(x)]=c\\cdot\\lim_{n\\to\\infty}f(x)$'}
        </div>
        <div>
          {
