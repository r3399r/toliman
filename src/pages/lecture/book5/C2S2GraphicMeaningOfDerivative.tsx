  ...
  return (
    <div>
      ...
      <Concept num={1} title="函數的增減與極值">
        ...
        <Textarea2MathJax
          text={'(1) 函數 $f(x)$ 滿足「若 $a<b$，則 $f(a)\\le f(b)$」，稱 $f(x)$ 為遞增'}
        />
        <Textarea2MathJax
          text={'(2) 函數 $f(x)$ 滿足「若 $a<b$，則 $f(a)<f(b)$」，稱 $f(x)$ 為嚴格遞增'}
        />
        <Textarea2MathJax
          text={'(3) 函數 $f(x)$ 滿足「若 $a<b$，則 $f(a)\\ge f(b)$」，稱 $f(x)$ 為遞減'}
        />
        <Textarea2MathJax
          text={'(4) 函數 $f(x)$ 滿足「若 $a<b$，則 $f(a)>f(b)$」，稱 $f(x)$ 為嚴格遞減'}
        />
        <Textarea2MathJax
          text={'圖形：遞增、遞減的函數圖形可以整段為水平，而嚴格遞增、嚴格遞減的函數圖形則否。'}
        />
        <div className={style.conceptHead}>極大值、極小值、最大值與最小值</div>
        <Textarea2MathJax
          text={
            '(1) 若 $f(a)$ 在 $x=a$ 附近是最大的，則稱 $f(x)$ 在 $x=a$ 有極大值 $f(a)$，又稱相對極大值'
          }
        />
        <Textarea2MathJax
          text={
            '(2) 若 $f(a)$ 在 $x=a$ 附近是最小的，則稱 $f(x)$ 在 $x=a$ 有極小值 $f(a)$，又稱相對極小值'
          }
        />
        <Textarea2MathJax
          text={'(3) 若 $f(a)$ 在定義域中是最大的，則稱 $f(x)$ 在 $x=a$ 有最大值 $f(a)$'}
        />
        <Textarea2MathJax
          text={'(4) 若 $f(a)$ 在定義域中是最小的，則稱 $f(x)$ 在 $x=a$ 有最小值 $f(a)$'}
        />
      ...
      <Example num={5} ids={['17c91d74236', '17c94b815cd']} />
      <Concept num={2} title="凹口方向與反曲點">
        <div className={style.conceptHead}>函數圖形的凹口方向</div>
        <Textarea2MathJax text={'函數圖形彎曲的部分看作一個虎口：'} />
        <Textarea2MathJax text={"(1) 虎口朝上稱為「凹向上」，因斜率遞增，滿足 $f''(x)>0$"} />
        <Textarea2MathJax text={"(2) 虎口朝下稱為「凹向下」，因斜率遞減，滿足 $f''(x)<0$"} />
